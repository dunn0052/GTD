...
namespace GTD
{
	bool PointInRect(const glm::vec2& p, const Rect& rect)
	{
	...
	bool RectCollideRect(const Rect& in, const Rect& other)
	{
		return
			(
	...
	bool RectCollideRay(const Rectangle& rect, const glm::vec2& rayOrigin, const glm::vec2& rayDir,
		glm::vec2& outContactPoint, glm::vec2& outContactNormal, float& t_hit_near)
	{
		...
		glm::vec2 t_far = (rect.Position() + rect.Size() - rayOrigin) * invRayDir;
	...
	bool MovableSolidCollideSolid(Movable& movable, Solid& solid, glm::vec2& outContactPoint, glm::vec2& outContactNormal, float& outContactTime, float& dt)
	{
		Rectangle expanded_target = Rectangle(solid.GetPosition() - movable.GetSize() * 0.5f, solid.GetSize() + movable.GetSize());
